# Store listing kit — KVSalaryTools

## Brand (ADR 0007)

| Field | Value |
|-------|--------|
| Name | KVSalaryTools |
| Subtitle / short | Ước tính lương · thuế · BHXH offline |
| Assistant | Ngài Miu — hướng dẫn breakdown & disclaimer |

## Screenshot set (phone 390×844)

Capture light mode only. Prefer device frame off for store upload; keep framed copies for design review.

| # | Screen | Why |
|---|--------|-----|
| 01 | Splash / onboarding intro (Ngài Miu) | Brand + assistant |
| 02 | Calculator empty → after Tính (Net hero) | Core job |
| 03 | Settlement result (hoàn / nộp thêm) | Trust / seasonal |
| 04 | Benefits hub | Poster menu |
| 05 | Retirement dual heroes (after disclaimer) | Serious benefit |
| 06 | Settings · Về chúng tôi | About + privacy |

## Copy snippets (vi)

**Short description**  
Ước tính Gross↔Net, quyết toán thuế và quyền lợi BHXH — offline trên thiết bị.

**Full description (opening)**  
KVSalaryTools giúp bạn xem rõ từng khoản trừ lương và thuế theo ruleset năm. Ngài Miu hướng dẫn từng bước; tính toán lưu cục bộ — không yêu cầu CCCD / MST.

## Assets

- App icon / splash: `assets/images/` (see also mascot pack when merged)
- Design QA: [`../product/design-qa-checklist.md`](../product/design-qa-checklist.md)

## Review captures

Cloud agent / local QA PNGs typically live under `/opt/cursor/artifacts/screenshots/` or `/opt/cursor/artifacts/store/` during polish runs — attach the latest set to the release PR.
