...
describe("ruleset other_income contract", () => {
  it("HKD industry_rates đủ 5 nhóm trên mọi ruleset", () => {
    ...
    expect(getRuleset(2025).casual_income!.withholding_threshold).toBe(
      2_000_000
    );
    expect(
      getRuleset(2026, "2026-03-15").casual_income!.withholding_threshold
    ).toBe(5_000_000);
    expect(
      getRuleset(2026, "2026-08-15").casual_income!.withholding_threshold
    ).toBe(5_000_000);
  });
});
