...
const FIXTURE_RULESET = {
  ...
  regional_minimum_wages: { '1': 5_310_000, '2': 4_730_000, '3': 4_140_000, '4': 3_700_000 },
  insurance_rates: {
...
function mockFetch(map: Record<string, { body: string; status?: number }>): typeof fetch {
...
describe('F019 remote ruleset update', () => {
  ...
  it('rejects bad checksum and keeps bundle', async () => {
    const before = listRulesets().map((r) => r.id).sort();
    const manifest = JSON.stringify({
      schemaVersion: 1,
      generatedAt: '2026-08-05T00:00:00.000Z',
      rulesets: [
    ...
    expect(listRulesets().map((r) => r.id).sort()).toEqual(before);
  });
