...
describe("calculateBonusMonth", () => {
  it("increases tax when bonus is added (same BH base)", () => {
    ...
    expect(result.withExtras.pit.totalTax).toBeGreaterThan(
      result.base.pit.totalTax
    );
    expect(result.deltaTax).toBe(
      result.withExtras.pit.totalTax - result.base.pit.totalTax
    );
