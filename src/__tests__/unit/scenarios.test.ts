...
describe('scenarios store (F014)', () => {
  ...
  it('parseSettlementInputs validates months and money', () => {
    ...
    expect(parseSettlementInputs({ ...sampleSettlement(), monthsWorked: 0 })).toBeNull();
    expect(parseSettlementInputs({ ...sampleSettlement(), monthlyGross: 0 })).toBeNull();
  });
  ...
  it('persists settlement scenarios alongside calculator', async () => {
    ...
    expect(scenariosOfKind(loaded.store.scenarios, 'settlement')).toHaveLength(1);
    expect(scenariosOfKind(loaded.store.scenarios, 'calculator')).toHaveLength(1);
  });
