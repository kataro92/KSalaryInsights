...
export function calculateSickLeave(input: SickLeaveInput): SickLeaveBreakdown {
  ...
  if (!params) throw new Error('Ruleset thiếu sick_leave');
  ...
  const explanations: string[] = [
    `Mức/ngày = ${params.rate * 100}% × ${input.salaryLastMonth.toLocaleString('vi-VN')} / ${params.divisor} = ${dailyRate.toLocaleString('vi-VN')}.`,
    `Trần năm (${hazard === 'hazardous' ? 'nặng nhọc' : 'bình thường'}, ${years} năm đóng) = ${annualCap} ngày.`,
  ];
  if (capped) {
    explanations.push(
  ...
  const formula = `${dailyRate.toLocaleString('vi-VN')}/ngày × ${daysPaid} ngày = ${amount.toLocaleString('vi-VN')} (${params.rate * 100}%/${params.divisor})`;
