...
export function validateRuleset(raw: unknown): Ruleset | null {
  ...
  if (typeof o.tax_year !== 'number' || !Number.isInteger(o.tax_year)) return null;
  if (o.tax_year < 2020 || o.tax_year > 2100) return null;
  if (!isDateLike(o.effective_from) || !isDateLike(o.effective_to)) return null;
  if (!Array.isArray(o.legal_sources) || o.legal_sources.length < 1) return null;
  if (!o.legal_sources.every(isNonEmptyString)) return null;
  if (!isFiniteNumber(o.personal_relief) || o.personal_relief < 0) return null;
  if (!isFiniteNumber(o.dependent_relief) || o.dependent_relief < 0) return null;
  if (!isFiniteNumber(o.reference_salary) || o.reference_salary < 0) return null;
  ...
  if (!isRate(emp.social) || !isRate(emp.health) || !isRate(emp.unemployment)) return null;
  if (!isRate(er.social) || !isRate(er.health) || !isRate(er.unemployment)) return null;

  const caps = o.insurance_caps;
  if (!caps || typeof caps !== 'object') return null;
  const c = caps as Record<string, unknown>;
  if (!isFiniteNumber(c.social_health_multiplier) || c.social_health_multiplier < 1) return null;
  if (!isFiniteNumber(c.unemployment_multiplier) || c.unemployment_multiplier < 1) return null;
  ...
  for (const br of o.pit_brackets) {
    ...
    if (typeof b.bracket !== 'number' || !Number.isInteger(b.bracket)) return null;
    if (b.max_taxable_income != null && !isFiniteNumber(b.max_taxable_income)) return null;
...
export function validateInflationTable(raw: unknown): InflationAdjustmentTable | null {
  if (!raw || typeof raw !== 'object') return null;
  const o = raw as Record<string, unknown>;
  if (typeof o.table_year !== 'number' || !Number.isInteger(o.table_year)) return null;
  if (!isNonEmptyString(o.legal_source)) return null;
  if (!o.coefficients_by_year || typeof o.coefficients_by_year !== 'object') return null;
