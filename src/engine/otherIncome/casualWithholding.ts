...
export function calculateCasualWithholding(
  ...
  if (!params) throw new Error("Thiếu tham số casual_income");
  ...
  const explanations: string[] = [
    `Ngưỡng khấu trừ theo mức ${ruleset.id.replace(
      /^ruleset-/,
      ""
    )}: ${threshold.toLocaleString("vi-VN")} (tỷ lệ ${rate * 100}%).`,
  ];

  let settlementWarning: string | undefined;
  if (withholdingApplied) {
    explanations.push(
      `Khấu trừ ${withheld.toLocaleString(
        "vi-VN"
  ...
  const formula = withholdingApplied
    ? `${rate * 100}% × ${input.paymentAmount.toLocaleString(
        "vi-VN"
      )} = ${withheld.toLocaleString("vi-VN")}`
