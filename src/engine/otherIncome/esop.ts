...
function resolveBookCost(input: EsopInput): { bookCost: number; usedFallback: boolean } {
  if (input.bookCostAtGrant != null && Number.isFinite(input.bookCostAtGrant)) {
    return { bookCost: Math.max(0, input.bookCostAtGrant), usedFallback: false };
  }
...
export function calculateEsop(input: EsopInput): EsopBreakdown {
  ...
  if (!params) throw new Error('Ruleset thiếu other_income.esop');
  ...
  const explanations: string[] = [
    usedFallback
      ? `Chi phí ghi sổ (fallback mệnh giá): max(0, shares×par − đã trả) = ${bookCost.toLocaleString('vi-VN')}.`
      : `Chi phí ghi sổ tại trao = ${bookCost.toLocaleString('vi-VN')}.`,
    `TLTC khấu trừ = ${params.tlcc_withholding_rate * 100}% × ${bookCost.toLocaleString('vi-VN')} = ${tlccWithholding.toLocaleString('vi-VN')}.`,
    `Thuế CN = ${params.transfer_rate * 100}% × ${input.salePrice.toLocaleString('vi-VN')} = ${transferTax.toLocaleString('vi-VN')}.`,
    settlementNote,
  ];
  ...
  return {
    ...
    effective: true,
    settlementNote,
    formula: `TLTC ${tlccWithholding.toLocaleString('vi-VN')} + CN ${transferTax.toLocaleString('vi-VN')} = ${totalTax.toLocaleString('vi-VN')}`,
    explanations,
    rulesetId: ruleset.id,
    legalSources: [
      ...ruleset.legal_sources,
