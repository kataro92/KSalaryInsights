...
export function calcPensionRate(
  ...
  if (sex === "female") {
    ...
    if (extra > 0) {
      steps.push(
        `+ ${extra} × ${params.female_increment_per_year * 100}% = ${(
          raw * 100
        ).toFixed(0)}%`
      );
    }
    if (raw > params.female_max_rate) {
      steps.push(
        `Trần ${params.female_max_rate * 100}% (@ ${
          params.female_max_years
        } năm)`
      );
    }
    return { rate, steps };
  ...
  if (years >= params.male_long_base_years) {
    ...
    steps.push(
      `Nam ≥${params.male_long_base_years} năm: ${
        params.male_long_base_rate * 100
      }% @ ${params.male_long_base_years} năm`
    );
    if (extra > 0) {
      steps.push(
        `+ ${extra} × ${params.male_long_increment_per_year * 100}% = ${(
          raw * 100
        ).toFixed(0)}%`
      );
    }
    if (raw > params.male_long_max_rate) {
      steps.push(
        `Trần ${params.male_long_max_rate * 100}% (@ ${
          params.male_long_max_years
        } năm)`
      );
    }
    return { rate, steps };
  ...
  if (extra > 0) {
    steps.push(
      `+ ${extra} × ${params.male_short_increment_per_year * 100}% = ${(
        rate * 100
      ).toFixed(0)}%`
    );
  }
...
export function calcPensionMonthly(input: PensionInput): PensionBreakdown {
  ...
  if (!params) throw new Error("Thiếu tham số pension_rates");
  ...
  const formula =
    rate === 0
      ...
      : `${(rate * 100).toFixed(0)}% × ${input.adjustedAvgSalary.toLocaleString(
          "vi-VN"
        )} = ${monthlyAmount.toLocaleString("vi-VN")}/tháng`;
