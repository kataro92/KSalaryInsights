...
import type { SeveranceBreakdown, SeveranceInput } from '@/src/domain/types/benefits';
...
function toMonths(years: number, extraMonths = 0): number {
  return Math.max(0, Math.trunc(years)) * 12 + Math.max(0, Math.trunc(extraMonths));
}
...
export function calcSeverancePay(input: SeveranceInput): SeveranceBreakdown {
  ...
  if (!params) {
    throw new Error('Ruleset thiếu severance_pay');
  }
  ...
  const formula =
    yearsCounted <= 0
      ...
      : input.mode === 'job_loss'
        ? `max(${rate} × ${yearsCounted} × ${input.avgSalary6m.toLocaleString('vi-VN')}, ${minMonths} × lương) = ${amount.toLocaleString('vi-VN')}`
        : `${rate} × ${yearsCounted} × ${input.avgSalary6m.toLocaleString('vi-VN')} = ${amount.toLocaleString('vi-VN')}`;
