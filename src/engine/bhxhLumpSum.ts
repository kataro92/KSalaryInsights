...
export function calcLumpSum(input: LumpSumInput): LumpSumBreakdown {
  ...
  if (!params) throw new Error('Ruleset thiếu lump_sum_withdrawal');
  ...
  if (totalMonths(input) < 12) {
    ...
    const amount =
      input.amountContributed != null
        ...
        : maxAmount;
    explanations.push(
      `Đóng chưa đủ 1 năm (${totalMonths(input)} tháng): mức = số đã đóng, tối đa ${params.under_one_year_max_months} tháng MBQTL = ${maxAmount.toLocaleString('vi-VN')}.`,
    );
    return {
      yearsPre2014Rounded,
      ...
      amount,
      underOneYear: true,
      formula: `min(đã đóng, ${params.under_one_year_max_months} × MBQTL) = ${amount.toLocaleString('vi-VN')}`,
      explanations,
      checklist,
      beforeCutoff,
  ...
  explanations.push(
    `Tổng hệ số năm = ${weightedYears} × MBQTL ${input.adjustedAvgSalary.toLocaleString('vi-VN')}.`,
  );

  const formula = `(${params.pre_2014_coefficient}×${yearsPre2014Rounded} + ${params.from_2014_coefficient}×${yearsFrom2014Rounded}) × ${input.adjustedAvgSalary.toLocaleString('vi-VN')} = ${amount.toLocaleString('vi-VN')}`;
