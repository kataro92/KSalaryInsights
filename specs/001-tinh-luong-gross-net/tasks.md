...
- [ ] T001 Khởi tạo cấu trúc các thư mục `src/domain/`, `src/engine/`, `src/components/`, `src/screens/` theo `plan.md`.
- [ ] T002 Define TypeScript contracts cho `SalaryInput`, `SalaryBreakdown`, `InsuranceBreakdown`, `PITBreakdown`, `Ruleset` trong `src/domain/types/salary.ts`.
- [ ] T003 [P] Tạo các file JSON ruleset `src/engine/rulesets/2025.json`, `2026-h1.json`, `2026-h2.json` tuân theo `docs/product/ruleset-schema.json`.
...
- [ ] T004 Implement `rulesetLoader.ts` trong `src/engine/rulesetLoader.ts` chọn ruleset theo `tax_year` và `as_of_date`.
- [ ] T005 [P] Implement `insurance.ts` tính đóng BHXH (8%), BHYT (1,5%), BHTN (1%) áp trần lương cơ sở và trần BHTN vùng trong `src/engine/insurance.ts`.
- [ ] T006 [P] Implement `pit.ts` tính thuế TNCN lũy tiến theo biểu 7 bậc (2025) và 5 bậc (2026) trong `src/engine/pit.ts`.
...
- [ ] T016 [P] [US3] Viết unit test kiểm thử trần bảo hiểm H1 (46,8tr) vs H2 (50,6tr) trong `src/__tests__/unit/insuranceCaps.test.ts`.

### Implementation for User Story 3
- [ ] T017 [US3] Bổ sung selector chọn "Tháng tính lương" trong `CalculatorScreen` để tự động chọn đúng ruleset H1/H2 năm 2026.

---

## Phase 6: Polish & UI Alignment (Flat Design + Ngài Miu)

- [ ] T018 [P] Thiết kế component linh vật `Ngài Miu` hiển thị lời khuyên / giải thích cơ cấu trừ lương trong `src/components/mascot/NgaiMiuTip.tsx`.
- [ ] T019 Kiểm tra giao diện theo thiết kế Flat Design trong `docs/product/design-system.md`.
